...
workTime float comment'稼働時間',
exceTime float comment'過稼働時間',
addpayOff int comment'加算額',
deficiTime float comment'不足稼働時間',
minusPayOff int comment'減算額',
transport int comment'交通費',
businessTrip int comment'出張旅費',
taxRate float comment'消費税率',
consumpTax int comment'消費税',
sum int comment'合計',
specialClaim int comment'特別請求',
claimStatus varchar(1) not null comment'請求ステータス',
claimFile varchar(50) not null comment'請求書',
insertDate varchar(8) comment'作成日',
updateDate varchar(8) comment'更新日'
)comment'請求情報';
...
CREATE TABLE bp_company (
    ...
    contact_person VARCHAR(10) COMMENT '連絡先名',
    email VARCHAR(20) COMMENT 'メール',
...
CREATE TABLE bp_contract (
    ...
    contract_name VARCHAR(50) COMMENT '契約名称',
...
CREATE TABLE bp_payment (
    ...
    unit_price_ex_tax INT COMMENT '取引単価--削除予定',
...
CREATE TABLE bp_invoice (
    ...
    file_name VARCHAR(50) COMMENT 'ファイル名フルパス',
