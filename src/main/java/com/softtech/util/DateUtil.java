...
public class DateUtil {
	 ...
	 * @author 馬@ソフトテク
	 */
		public static List<ListIDName> getYears(int yearNumber) {

			ArrayList<ListIDName> listIDNameList = new ArrayList<>();

			if (yearNumber < 1) return listIDNameList;

			LocalDateTime now = LocalDateTime.now();
		    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy");
		    //当年度
		    String thisYear = formatter.format(now);
		    ListIDName listIDName = new ListIDName();
		    listIDName.setYear(thisYear);
		    listIDNameList.add(listIDName);
		    String wkYear=thisYear;

		    for(int i=0;i<yearNumber-1;i++) {
		    	String oldYear =new BigDecimal(wkYear).subtract(BigDecimal.ONE).toString();
		    	ListIDName listIDNameOld = new ListIDName();
		    	listIDNameOld.setYear(oldYear);
		        listIDNameList.add(listIDNameOld);
		        wkYear=oldYear;
		    }
