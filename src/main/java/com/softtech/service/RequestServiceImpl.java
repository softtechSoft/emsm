...
public class RequestServiceImpl {
	...
	public List<RequestFromBean> transferEntityToFormBean(List<RequestEntity> ret1, String claimMonth) {
		if (ret1 == null)
			return new ArrayList<RequestFromBean>();
		...
		for (RequestEntity re : ret1) {
			...
			//契約時間
			requestDetail
					.setContractTime(requestDetail.getContractLowerTime() + "~" + requestDetail.getContractUpperTime());
	...
	public boolean processAndInsertClaims(String month) {
		try {
			...
			for (RequestEntity claimInfo : claimDatas) {
				...
				// Calculate sum
				int sum = (int) ((price + addpayOff - minusPayOff) * 1.1 + claimInfo.getTransport()
						+ claimInfo.getBusinessTrip());
	...
	public List<RequestToDownloadEntity> transforFormBeanToDownloadEntity(List<RequestFromBean> requestDetailList) {
		...
		for (RequestFromBean requestFromBean : requestDetailList) {
			...
			claimDownload.setContractID(requestFromBean.getContractID());
	...
	/**
	 * PDFファイルのパスをcontractテーブルのtimeReportPath列に更新する
	 * @param contractID 契約ID
	 * @param timeReportPath ファイルパス
	 */
	public void updateTimeReportPath(String contractID, String timeReportPath) {
		// MyBatisに引き渡すパラメータマップを作成
		Map<String, Object> paramMap = new HashMap<>();
		paramMap.put("contractID", contractID);
		paramMap.put("timeReportPath", timeReportPath);

		mapper.updateTimeReportPath(paramMap);
	}
	
	
	public String findTimeReportPathByContractID(String contractID) {
	    return mapper.findTimeReportPathByContractID(contractID);
	}
	
	public List<String> findContractIDsByWorkMonth(String workMonth) {
        return mapper.findContractIDByWorkMonth(workMonth);
    }
